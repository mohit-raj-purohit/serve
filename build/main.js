...
var package_default = {
  ...
  'lint-staged': {
    ...
    'source/**/*.ts': ['eslint --max-warnings 0 --fix', 'vitest related --run'],
    tests: ['vitest --run'],
  },
};
...
var startServer = async (endpoint, config2, args2, previous) => {
  const serverHandler = (request, response) => {
    const run = async () => {
      ...
      if (!args2['--no-compression']) await compress(request, response);
  ...
  const server = useSsl
    ...
    : http2.createServer(serverHandler);
...
var loadConfiguration = async (presentDirectory2, directoryToServe2, args2) => {
  ...
  if (directoryToServe2) {
    ...
    config2.public = resolveRelativePath(
      presentDirectory2,
      staticDirectory
        ? resolvePath(directoryToServe2, staticDirectory)
        : directoryToServe2,
    );
  }
